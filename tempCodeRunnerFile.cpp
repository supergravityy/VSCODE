...
void test_malloc(int C_Arr[])
{
    ...
    printf("The Array starting address is in C : %X", C_Arr);
    free(C_Arr);
...
void test_new(int Cpp_Arr[])
{
    ...
    printf("The Array starting address is in Cpp : %X", Cpp_Arr);
    delete[] Cpp_Arr;
...
int main()
{
    ...
    cout << "The Size of C Array : "<< sizeof(C_Arr) << endl;
    // 할당된 로컬 내에서 sizeof 함수에 배열의 시작주소를 입력하면, 인덱스의 숫자가 나온다.
